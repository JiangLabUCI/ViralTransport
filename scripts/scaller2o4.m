...
elseif strfind(filename,'adscnt')
    % Set kdec (for adscnt) to kgm (defined for firsto)
    p.kdec = p.kgm;
    p.decaytype = 'adscnt';
else 
...
savefilename = strcat('s2o4_',jobno,'_',modeltype,'_',num2str(n2run),'.mat');
disp(strcat('Saving results to : ', savefilename))
